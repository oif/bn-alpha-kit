...
let buyAmountEachTime = 2; // 每次买入数量
...
async function clickPriorityMode() {
  ...
  return waitForElement(
    () => {
      const priorityModeText = Array.from(
        document.querySelectorAll(".t-caption1")
      ).find((el) => el.textContent === "优先模式");
      if (priorityModeText) {
  ...
  return waitForElement(
    () => {
      const elements = document.querySelectorAll(
        ".p-\\[16px\\].rounded-\\[12px\\].border.border-solid.border-InputLine"
      );
      return Array.from(elements).find((el) =>
        el.textContent.includes("自定义")
      );
    },
    null,
    10,
...
async function setCustomSlippage(customSlippage) {
  // 查找并设置自定义滑点值
  return waitForElement("#customize-slippage", null, 10, 2000).then((el) => {
  ...
  return waitForElement(
    () => {
      const buttons = document.querySelectorAll(
        ".bn-button.bn-button__" + direction
      );
      for (const button of buttons) {
        if (!button.disabled) {
          return button;
...
async function openSpotHistory() {
  await openOrderHistory();
  return waitForElement(() => {
    const tab = document.querySelector("#trd-order-history #bn-tab-1");
    return tab;
  }).then((el) => {
    el.click();
    logit("已点击即时订单历史");
...
async function switchToSpotOperation() {
  return waitForElement(() => {
    return document.querySelector(
      '#bn-tab-INSTANT[aria-controls="bn-tab-pane-INSTANT"]'
    );
  }).then((el) => {
    el.click();
    logit(`已切换即时价格`);
...
async function launch() {
  ...
  for (let i = 0; i < loopCount; i++) {
    ...
    logit(
      "第" + (i + 1) + "次交易完成，预计交易额" + buyAmountEachTime * (i + 1)
    );
  }
}
...
async function waitForOrderProcessingCompletion() {
  ...
  return new Promise((resolve) => {
    const checkInterval = setInterval(() => {
      document.querySelector("#bn-tab-orderOrder").click();
      document.querySelector(".bn-tab-pane-list #bn-tab-instant").click();

      const pendingOrder = document.querySelectorAll(
        '.bn-tab-pane-list #bn-tab-pane-orderOrder .bn-web-table .bn-web-table-tbody>tr[role="row"]'
      ).length;
      if (pendingOrder === 0) {
        logit("订单处理已完成: 处理中(0)");
 ...
 * @returns {Promise<void>}
 */
async function placeTillSuccess(
  operationFunction,
  direction,
  maxRetryCount = retryOrderCheckMaxCount
) {
